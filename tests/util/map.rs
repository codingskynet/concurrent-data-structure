...
pub fn stress_sequential_btree(iter: u64)
{
    // 10 times try to get not existing key, or return if failing
    let gen_not_existing_key = |rng: &mut ThreadRng, map: &BTreeMap<String, u64>| {
        let mut key = String::gen(rng);
    ...
    for i in 1..=iter {
        let t = types.choose(&mut rng).unwrap();
        let ref_map_keys = ref_map.keys().collect::<Vec<&String>>();
        let existing_key = ref_map_keys.choose(&mut rng);
